...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { serviceName, cityName, cityProvince } = body

    if (!serviceName || !cityName) {
      return Response.json({ error: 'Servicio y ciudad son requeridos' }, { status: 400 })
    }

    // AI model - default to claude-sonnet-4 if not specified
    const selectedModel = body.model || 'openai/gpt-5-mini'
...
EJEMPLO COMPLETO DE INTRO_TEXT VALIDO (sigue este formato):

<h2>🔧 Por que elegir nuestro servicio de ${serviceName} en ${cityName}</h2>
<p>En <strong>${cityName}</strong> llevamos mas de <strong>10 anos</strong> ofreciendo servicios de calidad a nuestros vecinos. Conocemos cada rincon de la zona.</p>
<p><strong>Somos especialistas en emergencias.</strong> Nuestro equipo esta disponible 24 horas, los 7 dias de la semana, para atender cualquier urgencia que puedas tener.</p>
<p>🏠 Trabajamos en todas las zonas: desde el centro hasta los barrios residenciales, pasando por las urbanizaciones de las afueras.</p>

<h2>Ventajas de trabajar con nosotros</h2>
<p>Ofrecemos <strong>presupuestos sin compromiso</strong> y <strong>garantia por escrito</strong> en todos nuestros trabajos. Porque tu tranquilidad es nuestra prioridad.</p>
<p>Nuestros tecnicos son profesionales certificados con anos de experiencia en el sector. Utilizamos materiales de primera calidad y las tecnicas mas avanzadas del mercado para garantizar resultados duraderos.</p>

<h2>Servicio rapido y profesional 💪</h2>
<p><strong>Llegamos en menos de 45 minutos</strong> a cualquier punto de ${cityName}.</p>
<ul>
<li><strong>Sin costes ocultos</strong> - Presupuesto cerrado desde el inicio</li>
<li>Materiales de <strong>maxima calidad</strong></li>
<li>Garantia de satisfaccion</li>
</ul>
<p>No lo pienses mas y contacta con nosotros. <strong>Tu problema tiene solucion.</strong></p>
